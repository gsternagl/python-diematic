...
class Diematic:
  ...
  diematicReg = { \
    	...
    	'BOILER_TYPE': 	        (None, 457, INTEGER,  0.0), \
  ...
  def check_transmit_registers(self):
    ...
    if self.reg_isset(self.diematicReg, 'MODE_A'):
      ...
      if self.reg_getset(self.diematicReg, 'MODE_A') == self.ANTIICE:
        ...
        self.modBus.reg_set(self.diematicReg, 'NB_DAY_ANTIICE', 1)
	...
	if self.debug:
	  self.logger.log(self.modBus.log)
        ...
        # set anti-ice
        self.modBus.reg_set(self.diematicReg, 'NB_DAY_ANTIICE', 0)
	self.modBus.masterTx(self.regulatorAddress, self.diematicReg['NB_DAY_ANTIICE'])
	if self.debug:
	  self.logger.log(self.modBus.log)
  ...
  def synchro(self):
    ...
    while True:
      ...
      if self.debug:
        self.logger.log(self.modBus.log)
      if self.modBus.status != ModBus.FRAME_EMPTY or self.modBus.status != ModBus.READ_ERROR:
        break

    if self.debug:
      self.logger.log("Buffer empty\n")
    ...
    while i<500:
      ...
      if busStatus == self.SLAVE:
        ...
        if silentDetection == -1 and (self.modBus.status == ModBus.FRAME_OK or 
                                      self.modBus.status == ModBus.NOT_SUPPORTED_FC or
                                      self.modBus.status == ModBus.NOT_ADDRESSED_TO_ME):
          silentDetection = 0

        # update silent detection following context
        if silentDetection >= 0:
          if self.modBus.status == ModBus.FRAME_EMPTY or \
             self.modBus.status == ModBus.READ_ERROR:
            silentDetection += 1
          else:
            silentDetection = 0
...
#       self.modBus.masterRx(self.regulatorAddress, 384, 64)
#       if self.debug:
#         self.logger.log(self.modBus.log)
#       if self.modBus.status == ModBus.FRAME_OK:
#         self.dataDecode(self.modBus.rxReg)
#
#       # get 23 last registers starting at 448
#       self.modBus.masterRx(self.regulatorAddress, 448, 23)
#       if self.debug:
#         self.logger.log(self.modBus.log)
#       if self.modBus.status == ModBus.FRAME_OK:
#         self.dataDecode(self.modBus.rxReg)
       
        # set bus status into slave mode and re-arm silent detection
        busStatus = self.SLAVE
        silentDetection = -1
