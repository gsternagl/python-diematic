...
class Diematic(object):
    ...
    diematicReg = { \
          ...
          'LANGUAGE':        [None, 263, INTEGER,  True,  0, 0, 2], \
          'ALARM':           [None, 465, BIT,      False, 0x0, 0, 0] }


    # class constructor
    def __init__(self, conn, debug=False, logger=None):
        self.debug = debug
        self.conn = conn
        if logger != None:
            self.logger = logger
    ...
    def synchro(self):
        ...
        # open new connection
        self.mod_bus = ModBus(self.slaveAddress, self.conn, logger=self.logger)
        ...
        while i < 500:
            ...
            # master mode
                ...
                self.mod_bus.master_rx(self.regulatorAddress, 128, 64)
                if self.mod_bus.status == ModBus.FRAME_OK:
                    self.data_decode(self.mod_bus.rx_reg)

                # get 64 registers starting at reg 256
                self.mod_bus.master_rx(self.regulatorAddress, 256, 64)
                if self.mod_bus.status == ModBus.FRAME_OK:
                    self.data_decode(self.mod_bus.rx_reg)

                # get 64 registers starting at reg 384
                #self.mod_bus.master_rx(self.regulatorAddress, 384, 64)
                #if self.mod_bus.status == ModBus.FRAME_OK:
                #    self.data_decode(self.mod_bus.rx_reg)
                
                ## get 23 last registers starting at 448
                #self.mod_bus.master_rx(self.regulatorAddress, 448, 23)
                #if self.mod_bus.status == ModBus.FRAME_OK:
                #    self.data_decode(self.mod_bus.rx_reg)
#
