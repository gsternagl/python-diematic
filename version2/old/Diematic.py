...
class Diematic:
  ...
  #                             set,  addr, type,     value, min, max,
  diematicReg = { \
    	'CTRL': 		(None, 3,   INTEGER,  0, 0, 9999), \
     	'HOUR':			(None, 4,   INTEGER,  0, 0, 23), \
    	'MINUTE':		(None, 5,   INTEGER,  0, 0, 59), \
    	'WEEKDAY':		(None, 6,   INTEGER,  0, 1, 7), \
    	'TEMP_EXT':		(None, 7,   REAL10,   0.0, -50.0, 150.0), \
    	'CONS_SUMWIN':		(None, 8,   REAL10,   0.0, 15.0, 30.5), \
    	'NB_DAY_ANTIICE':	(None, 13,  INTEGER,  0, 0, 99), \
    	'CONS_DAY_A':     	(None, 14,  REAL10,   0.0, 10.0, 30.0), \
    	'CONS_NIGHT_A':   	(None, 15,  REAL10,   0.0, 5.0,  30.0), \
    	'CONS_ANTIICE_A': 	(None, 16,  REAL10,   0.0, 0.5,  20.0), \
    	'MODE_A':         	(None, 17,  BIT,      0x0, 0, 2), \
    	'TEMP_AMB_A':     	(None, 18,  REAL10,   0.0, 0, 0), \
    	'STEEPNESS_A':     	(None, 20,  REAL10,   0.0, 0.0, 4.0), \
    	'TCALC_A':        	(None, 21,  REAL10,   0.0, 0, 0), \
    	'CONS_DAY_B':     	(None, 23,  REAL10,   0.0, 10.0, 30.0), \
    	'CONS_NIGHT_B':   	(None, 24,  REAL10,   0.0, 5.0, 30.0), \
    	'CONS_ANTIICE_B': 	(None, 25,  REAL10,   0.0, 0.5, 20.0), \
    	'STEEPNESS_B': 	        (None, 29,  REAL10,   0.0, 0.0, 4.0), \
    	'CONS_ECS':       	(None, 59,  REAL10,   0.0, 10.0, 80.0), \
    	'TEMP_ECS':       	(None, 62,  REAL10,   0.0, 0.0,  150.0), \
        'CONS_BOILER':          (None, 74,  REAL10,   0.0, 0.0, 0.0), \
    	'TEMP_BOILER':      	(None, 75,  REAL10,   0.0, 0.0, 150.0), \
    	'BASE_ECS':       	(None, 89,  BIT,      0x0, 0, 0), \
    	'CONS_ECS_NIGHT': 	(None, 96,  REAL10,   0.0, 0.0, 15.0), \
    	'DAY':            	(None, 108, INTEGER,  0, 1, 31), \
    	'MONTH':          	(None, 109, INTEGER,  0, 1, 12), \
    	'YEAR':           	(None, 110, INTEGER,  0, 0, 99), \
    	'WATER_PRESSURE': 	(None, 456, REAL10,   0.0, 0.0, 100.0), \
  ...
  def reg_unset(self, reg, key):
    ...
    reg_min   = reg[key][4]
    reg_max   = reg[key][5]
#    reg.update( {key: (reg_set, reg_addr, reg_type, reg_value, reg_min, reg_max)} )
  ...
  def dataDecode(self, modBusReg):
    for key in modBusReg:
      ...
      for reg in self.diematicReg:
        ...
        reg_min  = self.diematicReg.get(reg)[4]
        reg_max  = self.diematicReg.get(reg)[5]
        ...
        if reg_addr == key:
          ...
          self.diematicReg.update({reg: (reg_set, reg_addr, reg_type, reg_value, reg_min, reg_max) } )
          break
