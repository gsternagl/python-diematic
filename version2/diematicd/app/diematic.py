...
from time import sleep, time as mytime
import logging
...
class Diematic:
  ...
  def synchro(self):
    ...
    if self.modBus.conn_fp is None or \
       self.modBus.status == ModBus.NOT_CONNECTED:
      self.logger.error("Can't open ModBus connection")
      return


    self.logger.debug("Connection Status: " + str(self.modBus.status))
    ...
    while i<500:
      ...
      if busStatus == self.SLAVE:
        # log
        self.logger.debug("Index:"+str(i)+" Bus Status : Slave Silence Detection :"+str(silentDetection))
