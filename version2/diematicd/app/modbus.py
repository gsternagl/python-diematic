...
class ModBus(object):
    ...
    def __init__(self, modbus_addr, conn):
    ...
    def master_rx(self, modbus_addr, reg_addr, reg_nb):
        ...
        tx = chr(modbus_addr) + chr(self.READ_ANALOG_HOLDING_REGISTERS) + \
             self.int2str(reg_addr) + self.int2str(reg_nb)
        ...
        while True:
            ...
            self.rx_buff = self.conn_read(self.MAX_BYTE_READ)
            i += 1
            if self.rx_buff is not None and self.rx_buff != '':
                break
        ...
        for i in range(0, reg_nb):
            idx = reg_addr + i
            val = 0x100 * ord(self.rx_buff[2*i+3]) + ord(self.rx_buff[2*i+4])
            if val >= 0x8000:
                val = -(val & 0x7fff)
            self.rx_reg.update({idx: val})
    ...
    def master_tx_n(self, modbus_addr, register):
        ...
        # prepare request frame
        nb = len(register)
        tx = chr(modbus_addr) + chr(self.WRITE_MULTIPLE_REGISTERS) + \
        ...
        crc = 0x100 * ord(buff[frame_len-1]) + ord(buff[frame_len-2])
        if crc != crc_calc:
            self.logger.debug("CRC_ERROR")
            self.status = self.CRC_ERROR
            return self.status
    ...
    """
    def int2str(self, int_val):
        return chr((int_val >> 8) & 0xff) + chr(int_val & 0xff)
